...
const Dstruyentrinhtham = () => {
  const navigation = useNavigation();
  const [stories, setStories] = useState([]);

  useEffect(() => {
    axios
      .get("http://192.168.100.45:3000/Tb_dstruyentrinhtham")
      .then((response) => {
  ...
  const handleOnPresssa = (id) => {
    const truyena= findStoryById(stories, id);
    if (truyena !== null) {
  ...
  return (
    <View style={{ flex: 1, marginTop: 20 }}>
      <View style={{ flexDirection: "row" }}>
        ...
        <Text style={{ fontSize: 25, color: "red", marginLeft: 120, marginTop: 30 }}>Trinh thám</Text>
      </View>
      <View style={{ flex: 1, marginTop: 20 }}>
        <FlatList
          data={stories}
...
export default Dstruyentrinhtham;