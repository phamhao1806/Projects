...
const Dstruyenhaihuoc = () => {
  const navigation = useNavigation();
  const [stories, setStories] = useState([]);

  useEffect(() => {
    axios
      .get("http://192.168.100.45:3000/Tb_dstruyenhaihuoc")
      .then((response) => {
  ...
  return (
    <View style={{ flex: 1, marginTop: 20 }}>
      <View style={{ flexDirection: "row" }}>
        ...
        <Text style={{ fontSize: 25, color: "red", marginLeft: 120, marginTop: 30 }}>Hài hước</Text>
      </View>
      <View style={{ flex: 1, marginTop: 20 }}>
        <FlatList
          data={stories}
...
export default Dstruyenhaihuoc;